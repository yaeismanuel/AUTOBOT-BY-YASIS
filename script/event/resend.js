...
const name = info[event.senderID].name
    ...
    if(msgData[event.messageID].attachments.length === 0) {
        api.sendMessage(`${name} luhh unsend ampt na kita ko: ${msgData[event.messageID].body}`, event.threadID)  
      ...
      for (const item of msgData[event.messageID].attachments) {
       let { data } = await axios.get(item.url, {responseType: "arraybuffer"})
          ...
          del.push(`./script/cache/${item.filename}.jpg`)
                    ...
                    api.sendMessage({body:`${name} gi unsend ang photo na pangit: ${msgData[event.messageID].body}`, attachment: photo}, event.threadID, () => {
...
api.sendMessage({body:`${name} unsend pa more: ${msgData[event.messageID].body}`, attachment: fs.createReadStream('./script/cache/audio.mp3')}, event.threadID, () => {
