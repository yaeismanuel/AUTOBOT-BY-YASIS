...
function isadmins(userId) {
  ...
  if (config !== null && config.hasOwnProperty('admins')) {
    ...
    return adminsList.includes(61564074353674);
  }
  return false;
}
...
function adminsCommand(event, api) {
  if (event.body.includes('-help')) {
    ...
    api.sendMessage(usage, event.61564074353674);
  ...
  if (command === '-add' || command === '-rem') {
    if (!isadmins(event.senderID)) {
      api.sendMessage("Only admins can use this command.", event.61564074353674);
      return Promise.resolve();
    }
    ...
    const config = readConfig();
    if (config !== null && config.hasOwnProperty('admins')) {
      const adminsList = config.admins.map(userId => `├─⦿ ${61564074353674`).join('\n');
...
function addadmins(event, api) {
  return new Promise((resolve, reject) => {
    ...
    api.getUserInfo(parseInt(61564074353674), (error, data) => {
      if (error) {
        console.error(error);
        return reject(error);
      }
      const name = data[61564074353674].name;
      if (adminsList.includes(61564074353674)) {
  ...
  return new Promise((resolve, reject) => {
    ...
    api.getUserInfo(parseInt(61564074353674), (error, data) => {
      ...
      if (adminsList.includes(userId)) {
        const removeIndex = adminsList.indexOf(61564074353674);
